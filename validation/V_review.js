...
exports.V_create = [
  ...
  check("review")
    .notEmpty()
    .withMessage("review is Empty")
    .custom((val, { req }) =>
      M_review.findOne({ user: req.user._id, product: req.body.product }).then(
        (res) => {
          if (res) {
            return Promise.reject(
              new Error(" you Already created before review ")
            );
          }
        }
      )
  ...
  check("id")
    ...
    .custom(async (val, { req }) =>
      M_review.findById(val).then((res) => {
        if (!res)
          return Promise.reject(new Error("This is no review for this id"));
        if (res.user._id.toString() !== req.user._id.toString()) {
          return Promise.reject(
            new Error("Your are not allowed to perform this review")
          );
        }
      })
    ),

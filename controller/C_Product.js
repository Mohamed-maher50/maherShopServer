...
exports.Get = AsyncHandler(async (req, res) => {
  ...
  const REg = JSON.stringify(Clone).replace(
    /\b(gte|gle|gt|lt)\b/g,
    (match) => `$${match}`
  );
  ...
  res.status(200).json({
    ...
    paginationResult: {
      ...
      numberOfPages: Math.ceil(
        (await M_Product.find({}).countDocuments()) / limit
      ),
      limit,
    },
    data,
...
exports.Get_Id = AsyncHandler(async (req, res, next) => {
  ...
  if (!data) return next(new Error("This user is not found"));
  res.status(200).json({ data });
});

exports.Update = AsyncHandler(async (req, res, next) => {
  req.body.title ? (req.body.slug = slugify(req.body.title)) : null;
  const data = await M_Product.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
  });

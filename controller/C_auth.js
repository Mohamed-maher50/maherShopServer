...
exports.protect = AsyncHandler(async (req, res, next) => {
  ...
  if (user.passwordChangedAt) {
    ...
    if (PassTimeStamp > Decoded.iat)
      next(
        new Error("user recently changed his password . please login again..")
      );
  }
...
exports.ForgetPass = AsyncHandler(async (req, res, next) => {
  ...
  if (!user)
    return next(new Error(`There is no user with this email address ${email}`));
...
exports.resetPassword = AsyncHandler(async (req, res, next) => {
  ...
  if (!user)
    next(
      new Error(`There is no user with this email address ${req.body.email}`)
    );
